...
namespace NUITizenGallery
{
    ...
    public class WebViewPageContentPage : ContentPage
    {
        ...
        public WebViewPageContentPage(Window win)
        {
            window = win;
            window.BackgroundColor = new Color((float)189 / 255, (float)179 / 255, (float)204 / 255, 1.0f);
            ...
            window.Add(simpleWebView);

            window.KeyEvent += Instance_KeyEvent;

            // load local files.
            websites[0] = $"file://{CommonResource.GetResourcePath()}webview/test1.html";
            websites[1] = $"file://{CommonResource.GetResourcePath()}webview/test2.html";
        ...
        protected void Deactivate()
        {
            ...
            window.Remove(forwardButton);
            window.Remove(refreshButton);
            window.Remove(addressBar);

            if (screenshotView != null)
            {
                window.Remove(screenshotView);
            }
        ...
        private bool OnTick(object sender, EventArgs e)
        {
            ...
            {
                window.Remove(hitTestImageView);
        ...
        private void OnHttpRequestIntercepted(WebHttpRequestInterceptor interceptor)
        {
            ...
            {
                string path = CommonResource.GetResourcePath() + "webview/Tizen.NUI.WebViewTest.png";
    ...
    class WebViewTest3 : IExample
    {
        ...
        public void Activate()
        {
            ...
            window.GetDefaultNavigator().Push(new WebViewPageContentPage(window));
        }
